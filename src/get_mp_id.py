import json
from tqdm import tqdm

from pymatgen.ext.matproj import MPRester
import pickle


def get_all_mp_id(api_key="OnQDHiVv3hzqx6p2"):
    mpr = MPRester(api_key)
    element_lst = [ 'H', 'He', 'Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne', 'Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'Ar', 'K', 'Ca', 'Sc', 'Ti', 'V',
    'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Zn', 'Ga', 'Ge', 'As', 'Se', 'Br', 'Kr', 'Rb', 'Sr', 'Y', 'Zr', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd', 'Ag',
    'Cd', 'In', 'Sn', 'Sb', 'Te', 'I', 'Xe', 'Cs', 'Ba', 'La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm', 'Eu', 'Gd', 'Tb', 'Dy', 'Ho', 'Er', 'Tm', 'Yb', 'Lu',
    'Hf', 'Ta', 'W', 'Re', 'Os', 'Ir', 'Pt', 'Au', 'Hg', 'Tl', 'Pb', 'Bi', 'Po', 'At', 'Rn', 'Fr', 'Ra', 'Ac', 'Th', 'Pa', 'U', 'Np', 'Pu', 'Am', 'Cm'
    , 'Bk', 'Cf', 'Es', 'Fm', 'Md', 'No', 'Lr', 'Rf', 'Db', 'Sg', 'Bh', 'Hs', 'Mt', 'Ds', 'Rg', 'Cn', 'Nh', 'Fl', 'Mc', 'Lv', 'Ts', 'Og']
    entries = mpr.query({"elements": {"$in": element_lst}}, ["material_id"])
    oxide_mp_ids = [e['material_id'] for e in entries]
    # oxide_mp_ids = [e for e in range(1000)]

    print(oxide_mp_ids)
    with open("all_mp_id.pkl", "wb") as f:
        pickle.dump(oxide_mp_ids, f)
    print(len(oxide_mp_ids))
    # with open("all_mp_id.pkl", "rb") as f:
    #     oxide_mp_ids = pickle.load(f)


if __name__ == "__main__":
    get_all_mp_id()